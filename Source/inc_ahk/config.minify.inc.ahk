...
g_actionListDBfileAdress := (InStr(A_ComputerName,"540P-SL5NET")) 	? "G:\fre\private\sql\sqlite\actionList.db"  	: A_ScriptDir "\actionListLearned.db"  
...
g_config.tray := {   	iconFileAddress: ( 		( InStr(A_ScriptDir,"-Nightly-")  		&& FileExist(A_ScriptDir "\icon\abc123\GI-nightly.gif") )  			? A_ScriptDir "\icon\abc123\GI-nightly.gif"  			: ""  	)  }  
...
g_config.sql := { 	select: {  		ignReg: "m)^((?!\bActionListID\b\s*[><=]).)*$" , 		ignIfWhereIsWithoutListID : false, 		showFilePrefix : false  	}, 	template : { 		dir: A_ScriptDir "\sql\template", 		maxNnumberUsedTemplates : 7 	}, 	DB_BrowserX86: A_ProgramFiles " (x86)\DB Browser for SQLite\DB Browser for SQLite.exe", 	DB_Browser: A_ProgramFiles "\DB Browser for SQLite\DB Browser for SQLite.exe" } 
