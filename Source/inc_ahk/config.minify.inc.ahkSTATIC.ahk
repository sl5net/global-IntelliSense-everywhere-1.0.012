...
g_actionListDBfileAdress := "C:\gi\Source" "\actionListLearned.db"
g_actionListDBfileAdress := (InStr(A_ComputerName,"540P-SL5NET")) 	? "G:\fre\private\sql\sqlite\actionList.db"  	: "C:\gi\Source" "\actionListLearned.db"  
...
g_config.ScriptDir := "C:\gi\Source"
...
g_config.actionList := {  	autoSwapping: false, 	tipps: { 		durationMilliseconds: 3500 	} }
g_config.list := { 	change: { stopRexExTitle: false } }
; if g_config.actionList.autoSwapping == false actionList not change by program automatically (for e.g. you change contect)
;  g_config.actionList.autoSwapping == true is buggy. thats way we enable to switch it off (false) 2020
;
...
g_config.tray := {   	iconFileAddress: ( 		( InStr("C:\gi\Source","-Nightly-")  		&& FileExist("C:\gi\Source" "\icon\abc123\GI-nightly.gif") )  			? "C:\gi\Source" "\icon\abc123\GI-nightly.gif"  			: ""  	)  }  
actionList_isNotAProject_withoutExt  := removesSymbolicLinksFromFileAdress( "C:\gi\Source" "\..\actionLists\_globalActionListsGenerated\isNotAProject" )
...
g_config.sql := { 	select: {  		ignReg: "m)^((?!\bActionListID\b\s*[><=]).)*$" , 		ignIfWhereIsWithoutListID : false, 		showFilePrefix : false  	}, 	template : { 		dir: "C:\gi\Source" "\sql\template", 		maxNnumberUsedTemplates : 7 	}, 	DB_BrowserX86: A_ProgramFiles " (x86)\DB Browser for SQLite\DB Browser for SQLite.exe", 	DB_Browser: A_ProgramFiles "\DB Browser for SQLite\DB Browser for SQLite.exe" } 
...
g_config.FuzzySearch := { 	enable: true, 	MAXlines : 87654,  	keysMAXperEntry : 6, 	minKeysLen: 4, 	doValueCopy : false } 
...
g_config.codeRunner := { 	code_AutoHotkey_Community:  	"C:\gi\Source" "\..\actionLists\_globalActionLists\incDynAhk\AutoHotkey_Community\in_commandLine_send_modified_clipboard.ahk", 	helloCommandLine: "C:\gi\Source" "\plugins\ahk\helloCommandLine.ahk", 	everything: A_ProgramFiles "\_\Everything\Everything.exe", 	everythingEs: A_ProgramFiles "\_\Everything\ES\es.exe", 	javascript: "node", 	php731NTS: "C:\php731NTS\php.exe",  	php: "C:\php\php.exe", 	python: "python", 	perl: "perl", 	ruby: "C:\Ruby23-x64\bin\ruby.exe", 	go: "go run", 	html: "C:\Program Files (x86)\Google\Chrome\Application\chrome.exe", 	java: "cd $dir && javac $fileName && java $fileNameWithoutExt", 	c: "cd $dir && gcc $fileName -o $fileNameWithoutExt && $dir$fileNameWithoutExt", 	kotlinc: "C:\gi\Source" "\..\actionLists\_kts\kotlinc\bin\kotlinc" } 
